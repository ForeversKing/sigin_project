...
class imdb(object):
    ...
    def append_flipped_up_down_images(self):
        num_images = self.num_images
        widths = self._get_widths()
        heights = self._get_heights()
        argument_num = 2
        for k in range(argument_num):
            for i in range(num_images):
                ...
                if k == 0:
                    ...
                    if 'gt_ishard' in self.roidb[i] and 'dontcare_areas' in self.roidb[i]:
                        ...
                        dontcare_areas[:, 0] = widths[i] - oldx2
                        dontcare_areas[:, 2] = widths[i] - oldx1
                    ...
                    if 'gt_ishard' in self.roidb[i] and 'dontcare_areas' in self.roidb[i]:
                        entry['gt_ishard'] = self.roidb[i]['gt_ishard'].copy()
                        dontcare_areas = self.roidb[i]['dontcare_areas'].copy()
                        oldy1 = dontcare_areas[:, 1].copy()
                        oldy2 = dontcare_areas[:, 3].copy()
                        dontcare_areas[:, 1] = heights[i] - oldy2
                        dontcare_areas[:, 3] = heights[i] - oldy1
                        entry['dontcare_areas'] = dontcare_areas
                    self.roidb.append(entry)
                else:
                    oldx1 = boxes[:, 0].copy()
        ...
        self._image_index = self._image_index * (argument_num+1)
